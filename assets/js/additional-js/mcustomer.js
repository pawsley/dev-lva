...
function menucustomer() {
    if (window.location.href === base_url+'customer/buat-baru') {
        ...
        editcst();
        deletedata();
    } else if (window.location.href === base_url+'master-agen'){
        tablecst();
        editcst();
        deletedata();
        addcst();
        getselect2();
    } else if (window.location.href === base_url+'master-tipe-agen'){
        newmod();
        editdatatipe();
        deletedatatipe();
        tabletipe();
...
function getselect2() {
    ...
    $('#tpc').on('change', function (e) {
        var selectedData = $('#tpc').select2('data'); 
    
        if (selectedData.length > 0) {
            $('#idtpc').val(selectedData[0].idsbc)
        }
    });
}
function newmod() {
    ...
    $('.shownewmod').on('click', function () {
        ...
        let labeldis = $(this).data('labeldis');
        
        // Set the modal content
        $('#titmod').text(title);
        $('#labmod').text(label);
        $('#labdis').text(labeldis);
        $('#addmod').off('click').on('click', function () {
            let item = $('#item').val();
            let valdis = $('#valdis').val();
            if (!item ) {
        ...
        success: function(response) {
            if (response.length===0) {
                ...
                $.each(response, function(index, daf) {
                    ...
                    var labelnm = $('<label class="form-label" for="item">Nama Tipe Agen</label>')
                    var inputField = $('<input class="form-control daf-name" data-id="'+daf.id_sbc+'" type="text">')
                                        .val(daf.nama_sbc);
                    var labelpr = $('<label class="form-label" for="item">Diskon Agen</label>')
                    var inputDis = $('<input class="form-control daf-dis" data-id="'+daf.id_sbc+'" type="number">')
                                        .val(daf.diskon);
                    var deleteButton = $('<button class="btn btn-danger deldaf" data-id="'+daf.id_sbc+'" type="button"><i class="fa fa-trash"></i></button>');
    
                    dafContainer.append($('<div class="col-5">').append(labelnm).append(inputField));
                    dafContainer.append($('<div class="col-5">').append(labelpr).append(inputDis));
...
function updatedaf() {
    $('#editmod').off('click').on('click', function (e) {
        ...
        $('#daf-container .daf-item').each(function() {
            ...
            var disk = $(this).find('.daf-dis').val();
...
function addcst() {
    $('#TambahAgen').on('shown.bs.modal', function () {
        generateid();
    });
...
function editcst() {
    $('#EditMasterCustomer').on('shown.bs.modal', function (e) {
        ...
        $('#etpc').on('change', function (e) {
            var selectedData = $('#etpc').select2('data'); 
        
            if (selectedData.length > 0) {
                $('#eidtpc').val(selectedData[0].idsbc)
            }
        });
        $('#estatus').select2({
            dropdownParent: $("#EditMasterCustomer"),
...
function editdatatipe() {
    $('#EditTipe').on('shown.bs.modal', function (e) {
        var button = $(e.relatedTarget);
        $('#eitem').val(button.data('nsbc'));
        $('#evaldis').val(button.data('disbc'));
        $('#form-edit').off('submit').on('submit', function(e) {
            e.preventDefault();
            $.ajax({
                type: 'POST',
                url: base_url + 'master-tipe-agen/update-data', 
                dataType: "json", 
                data: {
                    id: button.data('id'),
                    esbc: $('#eitem').val(),
...
function deletedatatipe() {
    $('#table-tipe').on('click', '#delete-data', function (e) {
        e.preventDefault();
        var id = $(this).data('id');
        swal({
            title: 'Apa anda yakin?',
            ...
            if (willDelete) {
                $.ajax({
                    type: 'POST',
                    url: base_url + 'master-tipe-agen/delete-data/' + id,
                    dataType: 'json',
                    success: function (response) {
                        if (response.success) {
                            swal('Deleted!', response.message, 'success');
...
function tabletipe() {
    $.getJSON(base_url+'assets/json/datatable-id.json', function(json) {
        ...
        tabtipe = $("#table-tipe").DataTable({
            ...
            "ajax": {
                "url": base_url+'master-tipe-agen/list-tipe-agen',
                "type": "POST"
            },
            "columns": [
                { "data": "nama_sbc"},
                    ...
                    "render": function (data, type, full, meta) {
                        if (type === "display") {
                            return `
                                    <ul class="action">
                                        <div class="btn-group">
                                            <button class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#EditTipe" 
                                            data-id="${data}" data-nsbc="${full.nama_sbc}" data-disbc="${full.diskon}"><i class="icon-pencil"></i></button>
                                            <button class="btn btn-secondary" id="delete-data" data-id="${data}"><i class="icon-trash"></i></button>
                                        </div>
                                    </ul>
                                `;
